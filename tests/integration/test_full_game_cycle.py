...
def test_full_game_cycle(mock_is_finished):
    ...
    # 3. Use Cases
    start_exp_uc = StartExpeditionUseCase(player_repo, exp_repo)
    start_battle_uc = StartBattleUseCase(player_repo, battle_repo)
    complete_exp_uc = CompleteExpeditionUseCase(exp_repo, player_repo, start_battle_uc)

    # 4. Игрок начинает вылазку
    start_exp_uc.execute("p1", ExpeditionDistance.NEAR)

    # 5. Вылазка завершается → запускается бой
    event = complete_exp_uc.execute("p1")
    
    assert isinstance(event, MonsterEncounter), f"Expected MosterEncounter, got {type(event)}"
    ...
    result = battle.perform_action("p1", BattleAction(BattleActionType.ATTACK))
