...
async def handle_battle_action(callback: CallbackQuery, async_session_maker):
    ...
    try:
        async with async_session_maker() as session:
            ...
            result = await use_cases["perform_battle_action"].execute(player_id, action)
            ...
            if result.is_finished:
                ...
                if result.requires_opponent_notification and result.opponent_id:
                    ...
                    try:
                        await callback.bot.send_message(
                            chat_id=int(result.opponent_id),
                            text=opponent_msg,
                            reply_markup=get_battle_keyboard(result.opponent_id)
                        )
